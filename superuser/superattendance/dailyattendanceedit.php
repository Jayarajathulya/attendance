...
$result = mysqli_query($conn, "SELECT * FROM attendance_taken WHERE id='" . $_GET['edit'] . "'");
...
if (isset($_POST['update'])) {
    ...
    $sql="select * from attendance_taken where  name ='$name' and department ='$department' and branchname='$branchname' and date='$date' and attendance='$attendance' ";
...
if (mysqli_num_rows($res) > 0) {
  ...
  if($name==isset($row['name']) && $department==isset($row['department']) && $branchname==isset($row['branchname']) && $date==isset($row['date']) && $attendance==isset($row['attendance']))
  {
    echo "<script type='text/javascript'>alert('Data Already Exists!!');location='dailyattendance.php'; </script>";
  }
  }
else{
//do your insert code here or do something (run your code)
$sql = "UPDATE attendance_taken set name='" . $_POST['name'] . "',department='" . $_POST['department'] . "' , country='" . $_POST['country'] . "' ,state='" . $_POST['state'] . "' ,city='" . $_POST['city'] . "' ,branchname='" . $_POST['branchname'] . "',date='" . $_POST['date'] . "',attendance='" . $_POST['attendance'] . "' WHERE id='" . $_GET['edit'] . "'";
$res=mysqli_query($conn,$sql);
echo "<script type='text/javascript'>alert('Data Updated successfully!!');location='dailyattendance.php'; </script>";
}
}
...
<body class="items-center bg-zinc-100">
    ...
    <div class="h-full mx-auto bg-zinc-100">
        <div class="px-2 py-2">
            ...
            <span class="text-3xl font-semibold text-center pb-4 text-sky-800">Daily Attendance Edit</span>
                ...
                <form action="" method="post">
                            ...
                            <div>
                                ...
                                <div>
                                    ...
                                    <input name="department" readonly value="<?php echo $row['department'] ?>"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5 ">
                                     
                                </div>
                                <div>
                                    <label for="date"
                                        class="block mb-1 text-sm font-medium text-gray-900 ">Date
                                    </label>

                                    <input name="date" readonly value="<?php echo $row['date'] ?>"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5 ">
                                     
                                </div>
                                <div>
                                    <label for="attendance"
                                        class="block mb-1 text-sm font-medium text-gray-900 ">Attendance
                                    </label>

                                    <input name="attendance" required value="<?php echo $row['attendance'] ?>"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full p-2.5 ">
