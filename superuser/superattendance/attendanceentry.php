...
$result = mysqli_query($conn, "SELECT date FROM `attendance_taken` WHERE  date = '$ThisDate' ");
...
?>
    <?php
    $result = mysqli_query($conn, "select count(1) FROM employee_details ");
    $row = mysqli_fetch_array($result);
    $total = $row[0];
    ...
    if (isset($_POST['submit'])) {
        for ($i = 0; $i < $total; $i++) {
            ...
            $branchcode = $_POST['branchcode'][$i];
                $sql = "INSERT INTO `attendance_taken`(`id`,`name`,`departmentname`,`state`,`city`,`facility`,`branchcode`,`date`,`attendance`) VALUES ('$id','$name','$departmentname','$state','$city','$facility','$branchcode','$date','$attendance')";
                        ...
                        <div class="w-full ">
                            <table class="w-full border border-separate rounded-t-lg border-spacing-1 overflow-x-auto">
                                ...
                                <tbody>
                                    ...
                                    $sql = "SELECT * FROM employee_details  LIMIT $offset, $no_of_records_per_page ";
